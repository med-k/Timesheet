pipeline {
    ...
    stages {
       stage ('GIT') {
        ...
        stage("DEPLOY : Nexus") {
            steps {
                bat "mvn clean package deploy:deploy-file -DgroupId=tn.spring -DartifactId=timesheet -Dversion=0.0.1 -DgeneratePom=true -Dpackaging=war -DrepositoryId=deploymentRepo -Durl=http://localhost:8055/repository/maven-releases/ -Dfile=target/timesheet-0.0.1.war"
            }
        }
